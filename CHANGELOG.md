...
- **HOR sweep post-processing now reports progress.** The previously silent 2D
  HTML rendering, compressed NPZ writing, and 3D HTML rendering phases each log
  when they start and finish, including elapsed time, output filename, and size.
  This makes large high-threshold sweeps visibly active after their native scans
  have completed.
...
- **HOR sweeps now report per-cutoff progress.** Each native scan logs its sweep
  index, threshold percentage (or collapsed percentage range), integer SNV
  cutoff, start, completion time, and number of HORs found. With multiple
  workers, completed scans appear as they finish rather than leaving a long
  silent interval after MAFFT.
